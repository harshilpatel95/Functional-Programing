(* NAMES:
 *
 * Partner 1's name: Brij Lathia(CIN:307401593)
 * Partner 2's name: Harshil Patel(CIN:306646748)
 *)
...
let read_page page = 
  ...
  let _ = set_binary_mode_in ch true in
  let n = in_channel_length ch in
  let data = really_input_string ch n in
    close_in ch ; data
;;
...
let std_response () = 
  std_response_header "text/html; charset=utf-8" ^ (read_page moogle_home_page)
;;

(* The header for search responses to clients. *)
let query_response_header = 
  std_response_header "text/html; charset=utf-8" ^ 
...
let send_std_response client_fd = 
 let s = std_response() in
  Unix.send client_fd (Bytes.of_string s) 0 (String.length s) []
;;
  ...
  in
    try 
      ...
      let response = 
        ...
        else if query_string = "moogle.jpg"
        then std_response_header "image/jpeg" ^ read_page "moogle.jpg"
        else if is_safe query_string 
        then std_response_header "text/html; charset=utf-8"
	         ^ read_page (local_path query_string)
        else (print "not safe!" ; std_response())
      in
      send_all client_fd (Bytes.of_string response)
...
let server (index:WordDict.dict) (ranks:RankDict.dict) = 
  ...
  let rec server_loop () = 
    ...
    let len = Unix.recv client_fd buf 0 (Bytes.length buf) [] in
    let request = Bytes.sub buf 0 len in
    let _ = process_request client_fd (Bytes.to_string request) index ranks in
