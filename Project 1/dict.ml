 ...
 struct
   ...
   let d =
     if (D.compare (fst w) (fst x) = Less) then
       Three(w_left, w, w_right, x, x_other)
     else if (D.compare (fst w) (fst x) = Greater) then
       Three(x_other, x, w_left, w, w_right)
     else
       failwith "Inserting duplicate" in
   Done(d)
   ...
   let balanced (d: dict) : bool =
     let rec helper (currD: dict): int option =
       ...
       | Leaf -> Some(1)
       | Two(d1, n, d2) ->
    (match helper d1, helper d2 with
     | Some(h1), Some(h2) ->
       if h1 = h2 then Some(h1 + 1)
       else None
     | _, _ -> None)
       | Three(d1, n1, d2, n2, d3) ->
    (match helper d1, helper d2, helper d3 with
     | Some(h1), Some(h2), Some(h3) ->
        if (h1 = h2) && (h2 = h3) then Some(h1 + 1)
        else None
     | _, _, _ -> None)
     in
     match helper d with
